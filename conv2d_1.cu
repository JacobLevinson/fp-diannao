...
/* same BX,BY,BZ you already settled on */
#define BX 16
#define BY 8
#define BZ 4
                                         ...
                                         VTYPE* neuron_n)
{
    ...
    int idx_n = blockIdx.z * blockDim.z + threadIdx.z;   // NEW
    ...
    VTYPE sum = 0.0f;                                    // scalar now

    for (int ky = 0; ky < Ky; ky++)
        for (int kx = 0; kx < Kx; kx++)
            for (int i = 0; i < Ni; i++) {
                int synapse_idx = ((ky * Kx + kx) * Nn + idx_n) * Ni + i;
                int neuron_i_idx = ((ky + idx_y) * NXPAD + kx + idx_x) * Ni + i;
                sum += synapse[synapse_idx] * neuron_i[neuron_i_idx];
            }

    int neuron_n_idx = ((yout * NXSCL + xout) * Nn + idx_n);
    neuron_n[neuron_n_idx] = (sum > 0) ? sum : sum / 4.0f;   // leaky-ReLU
}
...
//     // Launch the kernel
//     // dim3 blockDim(32, 32); // Hardcoded for Nx and Ny
...
//     dim3 gridDim( (Nx + BX - 1) / BX,          // ceil-div in X
//                   (Ny + BY - 1) / BY,          // ceil-div in Y
//                   (Nn + BZ - 1) / BZ );        // ceil-div in output channels
